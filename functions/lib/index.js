...
exports.apiAiRecall = exports.apiSync = exports.apiVault = exports.apiSearch = exports.apiAgentExecutions = exports.apiAgents = exports.apiKnowledgeGraph = exports.apiContextCapsule = exports.apiProjects = exports.apiTimeline = exports.apiSessions = exports.apiMemories = exports.apiAuthExtensionToken = exports.apiPaypalCancel = exports.apiPaypalReturn = exports.apiPaypalWebhook = exports.apiPaypalCreateSubscription = exports.apiPaypalSubscriptionStatus = exports.apiAuthFingerprint = exports.apiAuthSession = exports.apiAuthGoogle = void 0;
...
(0, v2_2.setGlobalOptions)({ region: 'us-central1', minInstances: 0 });
...
let cachedPayPalCreds = null;
async function getPayPalCredentials() {
    if (cachedPayPalCreds)
...
// ============================
exports.apiAuthGoogle = v2_1.https.onRequest(async (req, res) => {
...
// ============================
exports.apiAuthSession = v2_1.https.onRequest(async (req, res) => {
    if (handleCors(req, res))
        return;
    if (req.method !== 'GET')
        return res.status(405).json({ error: 'Method not allowed' });
...
// ============================
exports.apiAuthFingerprint = v2_1.https.onRequest(async (req, res) => {
    if (handleCors(req, res))
        return;
    try {
        if (req.method === 'GET') {
...
// ============================
exports.apiPaypalSubscriptionStatus = v2_1.https.onRequest(async (req, res) => {
    if (handleCors(req, res))
        return;
    if (req.method !== 'GET')
        return res.status(405).json({ error: 'Method not allowed' });
...
// ============================
exports.apiPaypalCreateSubscription = v2_1.https.onRequest(async (req, res) => {
    if (handleCors(req, res))
        return;
    if (req.method !== 'POST')
        return res.status(405).json({ error: 'Method not allowed' });
    try {
        ...
        const approveLink = (ppData.links || []).find((link) => link.rel === 'approve');
        if (!approveLink?.href)
            return res.status(500).json({ error: 'No approval URL' });
        res.set(corsHeaders(req.headers?.origin));
...
// ============================
exports.apiPaypalWebhook = v2_1.https.onRequest(async (req, res) => {
    if (handleCors(req, res))
        return;
    if (req.method !== 'POST')
        return res.status(405).json({ error: 'Method not allowed' });
...
// ============================
exports.apiPaypalReturn = v2_1.https.onRequest(async (req, res) => {
...
// ============================
exports.apiPaypalCancel = v2_1.https.onRequest(async (req, res) => {
    if (handleCors(req, res))
        return;
    if (req.method !== 'POST')
        return res.status(405).json({ error: 'Method not allowed' });
...
// ============================
exports.apiAuthExtensionToken = v2_1.https.onRequest(async (req, res) => {
    if (handleCors(req, res))
        return;
    if (req.method !== 'POST')
        return res.status(405).json({ error: 'Method not allowed' });
...
function emptyDataResponse(req, res, data) {
    res.set(corsHeaders(req.headers?.origin));
    return res.status(200).json({ data });
}
// GET /api/memories
exports.apiMemories = v2_1.https.onRequest(async (req, res) => {
    if (handleCors(req, res))
        return;
    const uid = await verifyUser(req);
    if (!uid)
...
// GET /api/sessions
exports.apiSessions = v2_1.https.onRequest(async (req, res) => {
    if (handleCors(req, res))
        return;
    const uid = await verifyUser(req);
    if (!uid)
...
// GET /api/timeline
exports.apiTimeline = v2_1.https.onRequest(async (req, res) => {
    if (handleCors(req, res))
        return;
    const uid = await verifyUser(req);
    if (!uid)
...
exports.apiContextCapsule = v2_1.https.onRequest(async (req, res) => {
    if (handleCors(req, res))
        return;
    const uid = await verifyUser(req);
    if (!uid)
...
exports.apiAgentExecutions = v2_1.https.onRequest(async (req, res) => {
    if (handleCors(req, res))
        return;
    const uid = await verifyUser(req);
    if (!uid)
...
// GET /api/search
exports.apiSearch = v2_1.https.onRequest(async (req, res) => {
    if (handleCors(req, res))
        return;
    const uid = await verifyUser(req);
    if (!uid)
...
// ============================
exports.apiAiRecall = v2_1.https.onRequest(async (req, res) => {
    if (handleCors(req, res))
        return;
    if (req.method !== 'POST')
        return res.status(405).json({ error: 'Method not allowed' });
    try {
        ...
        const memoryContext = relevantMemories
            .map((m) => `[${m.type || 'page'}] ${m.title || 'Untitled'}: ${(m.summary || m.content || '').substring(0, 200)}`)
            .join('\n');
        try {
