  ...
  try {
    ...
    if (!syncState) {
      return NextResponse.json({
        data: { deviceId, status: "unknown", syncVersion: 0, lastSyncAt: null, pendingOps: 0 },
      });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Failed to register device for sync" }, { status: 500 });
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (typeof syncVersion !== "number") {
      return NextResponse.json({ error: "syncVersion is required and must be a number" }, { status: 400 });
    }

    const existingState = await db.syncState.findUnique({ where: { deviceId } });
    ...
    const processedChanges: Array<{ type: string; id: string; status: string }> = [];
    if (Array.isArray(changes) && changes.length > 0) {
      for (const change of changes) {
        processedChanges.push({ type: change.type || "unknown", id: change.data?.id || "unknown", status: "processed" });
      }
    }
    ...
    const updatedState = await db.syncState.update({
      where: { deviceId },
      data: { syncVersion: newVersion, status: "synced", lastSyncAt: new Date(), pendingOps: 0 },
    });
