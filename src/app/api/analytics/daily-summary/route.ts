...
export async function GET(request: NextRequest) {
  try {
    ...
    const enrichedSummaries = summaries.map((summary) => {
      ...
      try {
        projects = summary.projects ? JSON.parse(summary.projects) : [];
      } catch {
        projects = [];
      }
    ...
    // Compute trends: current week vs previous week
    const now = new Date();
    ...
    const [currentWeekSummaries, previousWeekSummaries] = await Promise.all([
      db.aIDailySummary.findMany({
        where: { date: { gte: currentWeekStartStr, lte: currentWeekEndStr } },
      }),
      db.aIDailySummary.findMany({
        where: {
          date: { gte: previousWeekStartStr, lte: previousWeekEnd.toISOString().split("T")[0] },
        },
      }),
    ]);

    // Aggregate stats for comparison
    function aggregateWeekStats(
      weekSummaries: typeof currentWeekSummaries
    ) {
      let totalSessions = 0;
      let totalMemories = 0;
      ...
      for (const s of weekSummaries) {
        try {
          const stats = s.stats ? JSON.parse(s.stats) : {};
          totalSessions += stats.sessionsCreated || 0;
          totalMemories += stats.memoriesCreated || 0;
          totalDecisions += stats.decisionsMade || 0;
        } catch {
          // skip
        }

        try {
          const topics = s.topics ? JSON.parse(s.topics) : [];
        ...
        try {
          const projects = s.projects ? JSON.parse(s.projects) : [];
          for (const p of projects) {
            if (p.name) allProjects.add(String(p.name));
          }
        } catch {
          // skip
        }
      ...
      return {
        daysActive: weekSummaries.length,
        totalSessions,
        totalMemories,
        totalDecisions,
    ...
    const trends = {
      ...
      changes: {
        sessions: computeChange(
          currentStats.totalSessions,
          previousStats.totalSessions
        ),
        memories: computeChange(
          currentStats.totalMemories,
          previousStats.totalMemories
        ),
        decisions: computeChange(
          currentStats.totalDecisions,
          previousStats.totalDecisions
        ),
        activeDays: computeChange(
          currentStats.daysActive,
          previousStats.daysActive
        ),
      },
    };
