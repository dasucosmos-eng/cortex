  ...
  try {
    ...
    for (const insight of insights) {
      ...
      if (insight.type === "deep_work" && metric?.value) {
        totalDeepWorkHours += metric.unit === "minutes"
          ? metric.value / 60
          : metric.value;
      }
      ...
      if (
        insight.type === "productive_pattern" &&
        metric?.value &&
        metric.unit === "score"
      ) {
  ...
  try {
    ...
    if (!type || typeof type !== "string") {
      return NextResponse.json(
        {
          error:
            "type is required. Must be one of: deep_work, distraction, context_switch, productive_pattern, inefficiency, suggestion",
        },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { error: "Failed to create productivity insight" },
      { status: 500 }
    );
  }
}
