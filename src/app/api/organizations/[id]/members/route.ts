...
export async function POST(
  ...
  try {
    ...
    let parsedPermissions: string[] = [];
    try {
      parsedPermissions = JSON.parse(member.permissions || "[]");
    } catch {
      parsedPermissions = [];
    }

    return NextResponse.json(
      { data: { ...member, permissions: parsedPermissions } },
      { status: 201 }
    );
  } catch (error) {
...
export async function PUT(
  ...
  try {
    ...
    let parsedPermissions: string[] = [];
    try {
      parsedPermissions = JSON.parse(updatedMember.permissions || "[]");
    } catch {
      parsedPermissions = [];
    }

    return NextResponse.json({
      data: { ...updatedMember, permissions: parsedPermissions },
    });
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({
      data: { userId, organizationId: id, removed: true },
